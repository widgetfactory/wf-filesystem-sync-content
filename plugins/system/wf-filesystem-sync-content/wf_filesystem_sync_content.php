...
class PlgSystemWf_Filesystem_Sync_Content extends JPlugin
{
    ...
    private function searchAndReplaceFilename($before, $after, $type = 'files')
    {
        ...
        if ((int) $this->params->get('published_only', 1) == 1) {
    ...
    public function onWfFileSystemAfterRename($result)
    {
        if ($result->state) {
            ...
            $this->searchAndReplaceFilename($result->source, $result->path, $result->type);
        }
    }
